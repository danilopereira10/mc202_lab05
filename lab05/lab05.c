...
p_no allocateAtPosition(p_no list, int position, int size) {
	...
	for (p_no actual = first; actual != head; actual = actual->next) {
		if (actual->initialPosition <= position && actual->initialPosition + actual->freeSpace - 1 >= position) {
			if(!(actual->freeSpace >= size + (position - actual->initialPosition))) { //space not enuff;
				return NULL;
			} else {
				//allocate effectively
				int initialChoosenPosition = actual->initialPosition;
				actual->freeSpace = actual->freeSpace - size - (position - actual->initialPosition);
				actual->initialPosition = actual->initialPosition + size + (position - actual->initialPosition);
				if (actual->freeSpace == 0) {
...
int main() {
	...
	for (int i = 1; i <= m; i++) {
		...
		if (option == 'A') {
			...
			scanf("%d", &size_before);
			scanf("%d", &size_after);
			list = reallocate(list, initial_position, size_before, size_after);
		} else if (option == 'P') {
			if (!firstPrint) {
				printf("\nheap:");
				print_list(list);
